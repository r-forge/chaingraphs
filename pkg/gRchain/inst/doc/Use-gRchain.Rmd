...
vignette: >
  %\VignetteIndexEntry{Use-gRchain}
...
* Use backward selection to reduce the model. We do this with an infomration Criterion (BIC by default).
...
```
Once the var.frame is set up one ca fit the model by 
```{r,eval=FALSE}
data(cmc)
...
* `vartype`: This is an important argument especially for using the formual interface. When using the formula interface without a specified vartype , the function attempts to automatically detect the variable type from the data frame. This works fairly well for factors but is crude for metric variables. For teh latter it will always specify an OLS model. Setting vartype allows to fine tune the model used for the variables (see below). If this argumnet is used, then the order of characters in vartype must correspond to the variable in formula form left to right, or in var.frame form top to bottom.
